  ...
  return (
    <Card className="overflow-hidden">
      <Button
        type="button"
        variant="ghost"
        className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left normal-case tracking-normal text-sm font-medium text-muted-foreground hover:bg-foreground/[0.02]"
        onClick={() => setOpen((v) => !v)}
      >
        <span>{q}</span>
        <ChevronDown
    ...
    <div className="min-h-screen bg-black text-foreground px-4 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl">
        <Button asChild variant="link" className="mb-8 inline-flex text-sm font-semibold uppercase tracking-[0.28em] text-zinc-400 transition hover:text-foreground">
          <a href="/settings">{t("bill_back_to_home")}</a>
        </Button>

        <p className="text-[10px] uppercase tracking-[0.32em] text-muted-foreground">{t("bill_support_help_contact")}</p>
        <h1 className="mt-3 font-serif text-4xl tracking-tight text-foreground sm:text-5xl">{t("bill_support_center")}</h1>
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-3">
          {cards.map((c) => {
            ...
            return (
              <Card
                key={c.title}
                className="group overflow-hidden border-border/10 bg-foreground/[0.02] p-5 rounded-sm transition hover:border-border/30 hover:bg-foreground/[0.04]"
              >
                <a href={c.href} className="flex h-full flex-col">
                  <Icon className="h-6 w-6 text-muted-foreground" />
                  <h2 className="mt-4 text-base font-medium text-muted-foreground">{c.title}</h2>
                  <p className="mt-2 flex-1 text-sm leading-6 text-muted-foreground">{c.description}</p>
                  <span className="mt-4 text-xs uppercase tracking-[0.22em] text-muted-foreground group-hover:text-foreground transition-colors">
                    {c.action} →
                  </span>
                </a>
              </Card>
        ...
        {/* FAQ */}
        <div className="mt-16">
          <h2 className="font-serif text-2xl tracking-tight text-foreground">{t("bill_faq_title")}</h2>
        ...
        <p className="mt-16 text-sm text-muted-foreground">
          {t("bill_still_stuck")}{" "}
          <Button asChild variant="link" className="font-medium text-foreground hover:underline">
